+++
date = '2026-05-10T14:10:34+08:00'
draft = true
title = 'Libco'
+++
...
协程本质是**可以暂停和恢复执行的函数**, 暂停时需要保持 CPU 寄存器、栈等上下文信息，恢复时需要切换回之前的上下文。

### 1.1 上下文结构体: `coctx_t`
libco 用 `coctx_t` 结构体保持上下文：
```c
// libco/coctx.h
struct coctx_t
{
...
- ctx->ss_sp 是 malloc分配的栈底地址，ctx->ss_size 是栈大小。 栈指针 sp 从栈顶开始向下增长，栈顶放置函数地址（返回地址），当 `coctx_swap` 执行 `ret` 时会跳转到这个函数。

### 1.3 上下文切换：`coctx_swap()`
